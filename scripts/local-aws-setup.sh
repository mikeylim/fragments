...
# Stop the script if a required command fails.
set -eu
...
echo "Setting AWS environment variables for local AWS mocks"

export AWS_ACCESS_KEY_ID=test
echo "AWS_ACCESS_KEY_ID configured for local mocks"

export AWS_SECRET_ACCESS_KEY=test
echo "AWS_SECRET_ACCESS_KEY configured for local mocks"
...
until curl --silent --fail http://localhost:4566/_ministack/health |
...
echo "Local AWS setup complete: S3 bucket fragments and DynamoDB table fragments are ready"
