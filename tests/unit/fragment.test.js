...
const validTypes = [
	`text/plain`,
	/*
	 Originally, only text/plain was supported. Others are added by assignment.
*/
...
describe('Fragment class', () => {
	...
	describe('isSupportedType()', () => {
		...
		test('other types are not supported', () => {
			...
			expect(Fragment.isSupportedType('not a content type')).toBe(false);
	...
	describe('mimeType, isText', () => {
		...
		test('isText return expected results', () => {
			...
			const json = new Fragment({
				ownerId: '1234',
				type: 'application/json',
				size: 0,
			});
			expect(json.isText).toBe(false);
		});
	});
